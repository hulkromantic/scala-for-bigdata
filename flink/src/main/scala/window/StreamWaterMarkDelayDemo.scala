...
object StreamWaterMarkDelayDemo {
  // 线程安全的时间格式化对象
  ...
  def main(args: Array[String]): Unit = {
    ...
    val streams: DataStream[String] = env.socketTextStream("localhost", 9999)

    import org.apache.flink.api.scala._

    val data: DataStream[(String, Long)] = streams.map(fun = (data: String) => {
      // 输入数据格式：name:时间戳
      // flink:1559223685000
      try {
        val items: Array[String] = data.split(" ")
        (items(0), items(1).toLong)
      } catch {
        case _: Exception => println("输入数据不符合格式>>>" + data)
          ("0", 0L)
      }
    }).filter((data: (String, Long)) => !data._1.equals("0") && data._2 != 0L)

    //为数据流中的元素分配时间戳，并定期创建水印以监控事件时间进度
    val waterStream: DataStream[(String, Long)] = data.assignTimestampsAndWatermarks(assigner = new AssignerWithPeriodicWatermarks[(String, Long)] {
      ...
      override def getCurrentWatermark: Watermark = {
        // 允许延迟三秒
        val potentialWM: Long = currentMaxTimestamp - maxOutOfOrderNess
        // 保证水印能依次递增
      ...
      override def extractTimestamp(element: (String, Long), previousElementTimestamp: Long): Long = {
        ...
        val outData: String = String.format("key: %s    EventTime: %s    waterMark:  %s",
          element._1,
          sdf.format(time),
      ...
      .apply(new WindowFunction[(String, Long), String, Tuple, TimeWindow] {
        override def apply(key: Tuple, window: TimeWindow, input: Iterable[(String, Long)], out: Collector[String]): Unit = {
          ...
          while (iterator.hasNext) {
            val tup2: (String, Long) = iterator.next()
            timeArr.append(sdf.format(tup2._2))
          }

          val outData: String = String.format("key: %s    data: %s    startTime:  %s    endTime:  %s",
            key.toString,
            timeArr.mkString("-"),
