...
object TopN3 {
  def main(args: Array[String]): Unit = {
    ...
    val subjectAndTeacherAndCount: RDD[((String, String), Int)] = subjectAndTeacherAndOne.reduceByKey(partitioner, _ + _)

    //7.那么既然上面的返回的RDD各个分区内都是同一学科的数据,所以接下来直接对各个分区进行排序即可
    val result: RDD[((String, String), Int)] =
      subjectAndTeacherAndCount.mapPartitions(iter => {
        //进来的iter就是各个分区的数据,即各个学科的数据
        iter.toList.sortBy(_._2).reverse.take(1).toIterator
      })
  ...
  class SubjectPartitioner(subjects: Array[String]) extends Partitioner {
    //自定义一个分区规则(学科,分区编号)
    var partitionRule = mutable.Map[String, Int]()
    var num = 0;
    for (s <- subjects) {
    ...
    def getPartition(key: Any): Int = {
      ...
      val partitionNum = partitionRule(subject)
      partitionNum
    }
  }
